...
func OpenDBFile(dbPath string, pageSize int) (*DBFile, error) {
	...
	if err != nil {
		return nil, errorx.OpenFileFailed(dbPath)
	}
	defer fp.Close()

	fileInfo, err := fp.Stat()
	if err != nil {
		return nil, errorx.StatFileFailed(dbPath)
	}
	...
	if err != nil {
		return nil, errorx.ReadFileFailed(dbPath)
	}
	if n != pageSize {
		// fmt.Errorf("read %d bytes, expected %d", n, pageSize)
		return nil, errorx.IncompleteRead()
	}

	if bytes.Equal(buffer[:len(SQLiteHeader)-1], []byte(SQLiteHeader[:len(SQLiteHeader)-1])) {
		return nil, errorx.ErrAlreadyDecrypted()
	}
...
func DecryptPage(pageBuf []byte, encKey []byte, macKey []byte, pageNum int64, hashFunc func() hash.Hash, hmacSize int, reserve int, pageSize int) ([]byte, error) {
	...
	if !bytes.Equal(hashMac, pageBuf[hashMacStartOffset:hashMacEndOffset]) {
		return nil, errorx.ErrDecryptHashVerificationFailed()
	}

	iv := pageBuf[pageSize-reserve : pageSize-reserve+IVSize]
	block, err := aes.NewCipher(encKey)
	if err != nil {
		return nil, errorx.DecryptCreateCipherFailed()
	}
