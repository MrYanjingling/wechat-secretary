...
import (
	...
	"wechat-secretary/backend/wechat/decrypt/common"
...
func (d *V3Decryptor) Decrypt(ctx context.Context, dbfile string, hexKey string, output io.Writer) error {
	...
	if !d.Validate(dbInfo.FirstPage, key) {
		return errorx.ErrDecryptIncorrectKey()
	}
	...
	if err != nil {
		return errorx.OpenFileFailed(dbfile)
	}
	defer dbFile.Close()
	...
	if err != nil {
		return errorx.WriteOutputFailed()
	}
	...
	for curPage := int64(0); curPage < dbInfo.TotalPages; curPage++ {
		...
		select {
		case <-ctx.Done():
			return errorx.ErrDecryptOperationCanceled()
		default:
			// 继续处理
		}
		...
		if err != nil {
			...
			return errorx.ReadFileFailed(dbfile)
		}
