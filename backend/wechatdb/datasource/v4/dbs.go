...
import (
	...
	v4 "wechat-secretary/backend/wechatdb/model/v4"
...
type DataSource struct {
	...
	dbMap        map[string]Db
	dbChangeChan map[string]chan struct{}
...
func New(path string) (*DataSource, error) {
	...
	ds := &DataSource{
		...
		dbChangeChan: make(map[string]chan struct{}, 0),

		fgs: make(map[string]*filemonitor.FileGroup),
	}

	for _, g := range Groups {
		if db, ok := DBInitMaps[g.Name]; ok {
			ch := make(chan struct{})
			initDb, err := db.InitDb(path, g, ch)
			if err != nil {
				log.Errorf("Failed to init db err:%s", err)
			} else {
				ds.fgs[g.Name] = initDb
				ds.dbMap[g.Name] = db
				//
				ds.dbChangeChan[g.Name] = ch
			}
		}
	}
...
// 		name = Contact
// 	}
// 	return ds.db.AddCallback(name, callback)
// }
//
...
func (ds *DataSource) GetAllChatRooms(ctx context.Context) ([]*v4.ChatRoomModel, error) {
	...
	chatRooms := []*v4.ChatRoomModel{}
	for rows.Next() {
		var cr v4.ChatRoomModel
		err := rows.Scan(
			&cr.ID,
			&cr.UserName,
			&cr.Owner,
...
func (ds *DataSource) GetAllContact(ctx context.Context) ([]*v4.ContactModel, error) {
	var query string
	query = `SELECT username, local_type, alias, remark, nick_name, big_head_url, small_head_url, extra_buffer 
			 FROM contact`
	...
	contacts := []*v4.ContactModel{}
	for rows.Next() {
		var contact v4.ContactModel
		err := rows.Scan(
			&contact.UserName,
			&contact.LocalType,
			&contact.Alias,
...
func (ds *DataSource) GetContactLabel(ctx context.Context) ([]*v4.ContactLabelModel, error) {
	var query string
	query = `SELECT label_id_, label_name_, sort_order_ FROM contact_label`
	...
	contactLabels := []*v4.ContactLabelModel{}
	for rows.Next() {
		var contactLabel v4.ContactLabelModel
		err := rows.Scan(
			&contactLabel.LabelId,
			&contactLabel.LabelName,
			&contactLabel.SortOrder,
		)
		if err != nil {
			return nil, errorx.QueryFailed()
		}

		contactLabels = append(contactLabels, &contactLabel)
	}
...
func (ds *DataSource) Start() error {
	return ds.fm.Start()
}
...
func (ds *DataSource) Close() {
	for _, chs := range ds.dbChangeChan {
		close(chs)
	}

	for _, db := range ds.dbMap {
		db.Close()
	}
